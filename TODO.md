...
- [~] YAML/CSV asset inventory and local IOC enrichment are wired into scoring; opt-in AbuseIPDB/VirusTotal IP clients now have 5–6s timeouts and a SQLite-backed 24h cache, and the last five same-fingerprint incidents/verdicts are available to the LLM prompt; GeoIP and broader agent/src history remain.
