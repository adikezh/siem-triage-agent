...
func TestPollWazuhPersistsAlertAndIncident(t *testing.T) {
	...
	defer cancel()
	go pollWazuh(ctx, db, ingest.WazuhClient{BaseURL: srv.URL, Index: "alerts-*"}, time.Hour, 15*time.Minute, 6*time.Hour, config.Grouping{Default: []string{"rule.id", "agent.id", "src_ip"}}, nil, map[string]enrich.Asset{"203.0.113.8": {IP: "203.0.113.8", Criticality: 5}}, enrich.IOC{IPs: map[string]bool{"203.0.113.8": true}}, nil, nil, nil, "")
...
func TestPollWazuhRunsConfiguredEngine(t *testing.T) {
	...
	go pollWazuh(ctx, db, ingest.WazuhClient{BaseURL: srv.URL, Index: "alerts-*"}, time.Hour, 15*time.Minute, 6*time.Hour, config.Grouping{Default: []string{"rule.id", "agent.id", "src_ip"}}, nil, nil, enrich.IOC{}, nil, engine, nil, "")
	deadline := time.Now().Add(2 * time.Second)
	for time.Now().Before(deadline) {
		rows, e := db.ListIncidents(ctx)
		if e == nil && len(rows) == 1 {
